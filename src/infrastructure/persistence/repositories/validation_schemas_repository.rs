...
impl PipelineValidationSchemaRepository for PipelineStoreRepository {
    async fn save_pipeline_validation_schema(&self, schema: &PipelineNewValidateSchema) -> Result<(), IoTBeeError> {
        ...
        let schema_json = schema.schema().schema(); 
        ...
        match result {
            Ok(_) => Ok(()),
            Err(SqlxError::Database(db_error)) if db_error.is_unique_violation() => Err(IoTBeeError::from(
                PipelinePersistenceError::ValidationSchemaNameExists { name: schema.name().to_string() }
            )),

            Err(e) => Err(IoTBeeError::from(
                PipelinePersistenceError::SaveFailed { reason: e.to_string() }
    ...
    async fn delete_pipeline_validation_schema(&self, schema_id: &DataStroreId) -> Result<(), IoTBeeError> {
        ...
        let pool = self.data_base_connection().pool();
            ...
            .map_err(|e| IoTBeeError::from(
                crate::domain::error::PipelinePersistenceError::DeleteFailed { reason: e.to_string() }
            ))?;
    ...
    async fn update_pipeline_validation_schema(&self, schema_id: &DataStroreId, schema: &PipelineNewValidateSchema) -> Result<(), IoTBeeError> {
        ...
        sqlx::query(
            ...
            SET json_name = ?, json_schema = ?, updated_at = ?
            WHERE id = ?
            "#,
        )
        .bind(&schema.name())
    ...
    async fn get_pipeline_validation_schema(&self, schema_id: &DataStroreId) -> Result<Option<PipelineNewValidateSchema>, IoTBeeError> {
        ...
        .fetch_optional(pool)
        .await
        .map_err(|e| PipelinePersistenceError::Database { reason: e.to_string() })?;

        let result = row.map(|r| PipelineNewValidateSchema::try_from(r)).transpose()?;
        Ok(result)
    }
    ...
    //     // Implementation to check if a pipeline validation schema name exists in the database
    //     let pool = self.data_base_connection().pool();

    //     let rows = sqlx::query_as::<_, ValidationSchemaRow>(
    //         r#"
    //         SELECT 1
    //         FROM validation_schemas
    //         WHERE json_name = ?
