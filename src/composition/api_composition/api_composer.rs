...
impl AppState{
    ...
    pub fn connection_types_app_state(&self) -> web::Data<dyn ConnectionTypesUseCases + Send + Sync> {
        let type_connection_repo: Arc<ConnectionTypesRepository> = Arc::new(ConnectionTypesRepository::new(self.internal_data_base.clone()));
    ...
    pub fn validation_schemas_app_state(&self) -> web::Data<dyn SchemaValidationUseCases + Send + Sync> {
        let validation_schema_repo: Arc<ValidationSchemaRepository> = Arc::new(ValidationSchemaRepository::new(self.internal_data_base.clone()));
    ...
    pub fn data_sources_app_state(&self) -> web::Data<dyn DataSourcesUseCases + Send + Sync> {
        let data_sources_repo: Arc<DataSourceRepository> = Arc::new(DataSourceRepository::new(self.internal_data_base.clone()));
    ...
    pub fn pipeline_groups_app_state(&self) -> web::Data<dyn PipelineGroupUseCases + Send + Sync> {
        let pipeline_groups_repo: Arc<GroupRepository> = Arc::new(GroupRepository::new(self.internal_data_base.clone()));
    ...
    pub fn data_stores_app_state(&self) -> web::Data<dyn DataStoreUseCases + Send + Sync> {
        let data_stores_repo: Arc<DataStoreRepository> = Arc::new(DataStoreRepository::new(self.internal_data_base.clone()));
    ...
    pub fn pipeline_data_app_state(&self) -> web::Data<dyn PipelineDataUseCases + Send + Sync> {
        let pipeline_data_repo: Arc<PipelineDataRepository> = Arc::new(PipelineDataRepository::new(self.internal_data_base.clone()));
