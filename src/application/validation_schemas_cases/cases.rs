...
use crate::domain::entities::pipeline::PipelineNewValidateSchema;
...
use crate::application::validation_schemas_cases::validation_entities::{
    ValidationSchema, ValidationSchemaModel, ValidationSchemeModeById,
};
...
pub trait SchemaValidationUseCases {
    ...
    async fn create_validation_schema(
        &self,
        eschemas_model: &ValidationSchema,
    ) -> Result<(), IoTBeeError>;
    ...
    async fn get_validation_schema_by_id(
        &self,
        id: u32,
    ) -> Result<Option<ValidationSchemeModeById>, IoTBeeError>;
}
    ...
    T: PipelineGeneralRepository + Send + Sync,
{
    async fn create_validation_schema(
        ...
        let domain_schema =
            PipelineNewValidateSchema::new(schema_model.name(), schema_model.schema())?;
    ...
    async fn get_validation_schema(&self) -> Result<Vec<ValidationSchemaModel>, IoTBeeError> {
        ...
        let result: Vec<ValidationSchemaModel> = schemas
            .into_iter()
            .map(|s| {
    ...
    async fn get_validation_schema_by_id(
        ...
        let schema: Option<PipelineNewValidateSchema> = self
            .repository
