...
pub trait PipelineGroupUseCases {
    ...
    async fn create_pipeline_group(&self, group_name: &str, group_description: &str) -> Result<(), IoTBeeError>;

    async fn get_pipeline_groups(&self) -> Result<Vec<PipelineGroupOutputModel>, IoTBeeError>;
    async fn get_pipeline_group_by_id(&self, group_id: &u32) -> Result<PipelineGroupOutputModel, IoTBeeError>;
